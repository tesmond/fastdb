...
static DB: Lazy<Arc<Mutex<Connection>>> = Lazy::new(|| {
    let conn = Connection::open("fastdb.db").expect("Failed to open database");
...
pub fn update_server_last_connected(server_id: &str, timestamp: i64) -> Result<(), rusqlite::Error> {
    let conn = DB.lock().unwrap();
...
pub fn get_schemas(server_id: &str) -> Result<Vec<Schema>, rusqlite::Error> {
    ...
    let schemas = stmt.query_map([server_id], |row| {
        Ok(Schema {
            id: row.get(0)?,
            server_id: row.get(1)?,
            name: row.get(2)?,
...
pub fn get_tables(schema_id: &str) -> Result<Vec<Table>, rusqlite::Error> {
    ...
    let tables = stmt.query_map([schema_id], |row| {
        Ok(Table {
            id: row.get(0)?,
            schema_id: row.get(1)?,
            name: row.get(2)?,
...
pub fn batch_insert_tables(tables: &[Table]) -> Result<(), rusqlite::Error> {
    ...
    {
        let mut stmt = tx.prepare_cached(
            "INSERT INTO tables (id, schema_id, name, type) VALUES (?, ?, ?, ?)"
        )?;
...
pub fn get_query_history(server_id: &str, limit: usize) -> Result<Vec<QueryHistory>, rusqlite::Error> {
    let conn = DB.lock().unwrap();
    let mut stmt = conn.prepare_cached(
    ...
    let history = stmt.query_map(params![server_id, limit], |row| {
        Ok(QueryHistory {
            id: row.get(0)?,
            server_id: row.get(1)?,
            sql: row.get(2)?,
...
pub fn refresh_server_schema(
    ...
    // Batch insert new data
    {
        ...
        for schema in schemas {
            stmt.execute(params![schema.id, schema.server_id, schema.name, schema.last_updated])?;
        }
    }
...

    {
        ...
        for column in columns {
            stmt.execute(params![column.id, column.table_id, column.name, column.data_type, column.nullable])?;
        }
    }
...
pub fn is_schema_stale(server_id: &str, threshold_seconds: i64) -> Result<bool, rusqlite::Error> {
    let conn = DB.lock().unwrap();
    let mut stmt = conn.prepare_cached(
        "SELECT COUNT(*) FROM schemas WHERE server_id = ? AND last_updated < ?"
    )?;
